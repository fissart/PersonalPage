    ...
    import { Svc23TESISService } from '../../services/svc23-tesis.service';
    ...
    @Component({
      ...
      templateUrl: './cmpt23-tesis.component.html',
      styleUrls: ['./cmpt23-tesis.component.css']
    })
    export class Cmpt23TESISComponent implements OnInit {
      public title!: string;
      ...
      open1(ww: any, id: string, title: string, description: string, file: string, type: string, author: string) {
      this.id = id;
      this.title = title;
      this.subtype = type;
      this.img = file;
      ...
      updatenews(title: HTMLInputElement, description: HTMLTextAreaElement, author: HTMLInputElement) {
      this.Service.update(this.id, title.value, description.value, this.subtype, author.value, this.archivos[0])
      .subscribe(
      (res: any) => {
      this.loading = "false";
